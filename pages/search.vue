...
export default {
  ...
  watch: {
    getSearchKey(val) {
      this.onItemTap(this.currentIndex)
    }
  },
  ...
  methods: {
    ...
    async loadArticleList(page) {
      this.loading = true
      let row = await this.$axios.get('/api/v1/article/createtime', { params: { page: page, row: 20, status: 1, showAuthor: true, title: this.$store.state.searchKey } })
      let baseDisplay = []
    ...
    async loadBookList(page) {
      this.loading = true
      let row = await this.$axios.get('/api/v1/book', { params: { page: page, row: 20, status: 1, name: this.$store.state.searchKey } })
      let bookList = []
      if (row.data.code == 200) {
        for (let item of row.data.data) {
          bookList.push({
